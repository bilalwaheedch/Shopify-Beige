...
theme.config = {
  ...
  mediaQuerySmall: 'screen and (max-width: '+ 999 +'px)',
  youTubeReady: false,
  vimeoReady: false,
  vimeoLoading: false,
...
(function(){
    ...
    Variants.prototype = Object.assign({}, Variants.prototype, {
      ...
      _onSelectChange: function({srcElement}) {
        ...
        const detail = {
          ...
          console.log("detail:",detail.currentlySelectedValues.length)
        ...
        // console.log("srcElement.dataset.index:", srcElement.dataset.index)
        if (!variant) {
          return;
        }
        ...
        if (srcElement.dataset.index === 'option1' && detail.currentlySelectedValues.length > 1) {  // assuming 'color' is the dataset index for color variants
          location.reload();
        }
        // location.reload();
      },
  ...
  theme.AjaxRenderer = (function () {
    ...
    AjaxRenderer.prototype = Object.assign({}, AjaxRenderer.prototype, {
      renderPage: function (basePath, newParams, updateURLHash = true) {
        ...
        const formData = new FormData(document.querySelector('form#collection-fitlters-form'));
        const searchParamsSa = new URLSearchParams(formData);
        console.log("MMMMMM",searchParamsSa.toString())
        const sectionRenders = this.sections.map(section => {
  
          const url = `${basePath}?section_id=${section.sectionId}&${searchParamsSa.toString()}`;
  ...
  theme.collapsibles = (function() {
    ...
    function toggle(evt) {
      ...
      if (isOpen) {
        ...
        var elementsWithClass = document.querySelectorAll(".is-open");
        ...
        var getHeightElement = document.querySelectorAll(".collapsible-content.collapsible-content--all");
       
        getHeightElement.forEach(function(heightElement) {
      ...
      addItemFromForm: function(evt, callback){
        ...
        .then(function(data) {
          if (data.status === 422 || data.status == 'bad_request') {
            $(".select-styled").addClass("active");
            $(".layer-dropdown").addClass("show-layer");
  ...
  theme.QtySelector = (function() {
    ...
    function QtySelector(el, options) {
      ...
      this.remove = el.querySelector(selectors.remove);
  ...
  theme.Slideshow = (function() {
    ...
    slideshow.prototype = Object.assign({}, slideshow.prototype, {
      ...
      slideChange: function(index) {
        ...
        // let imageOffset = document.querySelectorAll(".flickity-viewport .image-wrap")[0].offsetHeight;
        // console.log('flickySliderEl after chage',flickySliderEl)
        // console.log('imageOffset after chage',imageOffset)
        // flickySliderEl[0].style.height = imageOffset+"px";
      },
  ...
  console.log('navDetailsnavDetailsnavDetails', navDetails);
      navDetails.forEach((navDetail) => {
        const summary = navDetail.querySelector('summary');
        if(!summary)return
  console.log('summarysummarysummarysummarysummary', summary);
  ...
  theme.HeaderCart = (function() {
    ...
    HeaderCart.prototype = Object.assign({}, HeaderCart.prototype, {
      ...
      close: function(evt) { 
        ...
        if(evt && evt.target.id == 'HeaderCart') {
  ...
  theme.Collection = (function() {
    ...
    Collection.prototype = Object.assign({}, Collection.prototype, {
      ...
      initPriceRange: function() {
        ...
        // theme.priceRange(priceRangeEls[0],{
      ...
      onReplaceAjaxContent: function(newDom, section) {
        ...
        var pcount = document.getElementById('count--label');
        if (countEl) {
          var count = countEl.innerText;
          document.querySelectorAll('[data-collection-count]').forEach(el => {
            el.innerText = count;
          });
          var mySpan = document.getElementById('p-count');
          mySpan.textContent = pcount.innerText;
        }
      ...
      renderFromFormData: function(formData) {
        ...
        console.log("LLL",searchParams.toString())
    ...
    Product.prototype = Object.assign({}, Product.prototype, {
      ...
      updateVariantImage: function(evt) {
        ...
        if (variant.featured_media && variant.featured_media.alt){
          ...
          const currentImgAlt = variant.featured_media.alt
  ...
  DOMready(function(){
    ...
    // Jquer Code ForFilter popup open with respective select filter
    ...
    //     $(`.accordion-header[id=${buttonId}]`).addClass('active')
    //     $(`.accordion-content[id=${buttonId}]`).show()
    ...
    // Filter popup open with respective select filter
...
//     const variantUrl = this.getAttribute('data-url');
//     const linkElement = this.closest('.grid-item').querySelector('.grid-item__link');
//     if (linkElement) {
...
//   // Select the first color swatch and add the selected class on load
//   const firstSwatch = document.querySelector('.color-swatch');
//   if (firstSwatch) {
//     firstSwatch.classList.add('selected');
//   }

//   // Add click event listener to all swatches
//   document.querySelectorAll('.color-swatch').forEach((swatch) => {
//     swatch.addEventListener('click', function () {
//       // Remove selected class from any currently selected swatches
//       document.querySelectorAll('.color-swatch.selected').forEach((el) => {
//         el.classList.remove('selected');
//       });
...
//       // Update the product link to the variant URL
//       const variantUrl = this.getAttribute('data-url');
//       const linkElement = this.closest('.grid-item').querySelector('.grid-item__link');
//       if (linkElement) {
//         linkElement.href = variantUrl;
...
//       const variantId = this.getAttribute('data-variant-id');
//       // You can use variantId here if you need it for any other actions
//     });
//   });
// });

document.addEventListener('DOMContentLoaded', function () {
  // Select the first color swatch for each product and add the selected class on load
  document.querySelectorAll('.grid-item').forEach((product) => {
    const firstSwatch = product.querySelector('.color-swatch');
    if (firstSwatch) {
      firstSwatch.classList.add('selected');
    }
  });
  ...
  document.querySelectorAll('.color-swatch').forEach((swatch) => {
    swatch.addEventListener('click', function () {
      // Remove selected class only from swatches within the current product container
      const productContainer = this.closest('.grid-item');
      productContainer.querySelectorAll('.color-swatch.selected').forEach((el) => {
        el.classList.remove('selected');
      });
      ...
      // Update the product link to the variant URL
      const variantUrl = this.getAttribute('data-url');
      const linkElement = productContainer.querySelector('.grid-item__link');
      if (linkElement) {
        linkElement.href = variantUrl;
      }

      const variantId = this.getAttribute('data-variant-id');
      // Use variantId here if needed for other actions
    });
  });
});
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Select the newsletter element by its class
  const newsletter = document.querySelector('.newsletter-container');

  if (newsletter) {
    // Apply CSS based on the condition
    if (currentUrl !== "https://beige.pk/") {
